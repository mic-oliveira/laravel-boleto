...
class BradescoBoletoService extends BilletEmissionService implements BoletoInterface
{
    ...
    public function authorize(): string
    {
        ...
        return (new AuthService())->authorize($this->signature);
    }
    ...
    private function makeSignature(array $data, string $url, string $verb = null)
    {
        try{
            ...
            $this->signature->setAccount(config('boleto.bradesco_account'));
            $this->signature->setAgency(config('boleto.bradesco_agency'));
            ...
            $this->signature->setBradSignature(
                SignatureService::bradRequestSignature(
                        $this->signature, config('boleto.bradesco_certificate_path', config('boleto.bradesco_certificate_pass')
                    )
                )
            );
        } catch (Exception $exception) {
    ...
    public function billetFromArray(array $data)
    {
        ...
        array_key_exists('email',$data['payer']) && !empty($data['email']) ? resolve(EmailRepository::class)
            ->createOrUpdate(['email' => $data['payer']['email'],'person_id' => $payer->id]) : null;
        ...
        array_key_exists('email',$data['payer']) && !empty($data['email']) ? resolve(EmailRepository::class)
            ->createOrUpdate(['email' => $data['drawer']['email'],'person_id' => $drawer->id]) : null;
        resolve(AddressRepository::class)
